...
const TableRow = ({
  ...
  return (
    <>
      <tr
        className={`group ${openItem === message?._id ? "bg-primary/10" : ""} hover:bg-primary/10 relative border-b duration-200 `}
        // onClick={async () => {
        //   if (!message?.is_seen) {
        ...
        // }}
      >
        <td className="py-6" colSpan={openItem === message?._id ? 2 : 1}>
          <button
          ...
          {/* Left Actions & Info */}
          <div className="flex items-center pl-2 md:gap-2">
        ...
        {openItem !== message?._id && (
          <td>
            <div>
              <h6 className="text-title max-h-4 max-w-40 overflow-hidden text-[10px] font-semibold md:max-w-80 md:text-xs">
                {message?.subject + "..."}
              </h6>
              <p
                className="max-h-4 max-w-40 overflow-hidden text-xs text-[#303133] md:max-w-80"
                dangerouslySetInnerHTML={{
                  __html: message?.content?.substring?.(0, 38) + "...",
                }}
              />
            </div>
          </td>
      ...
      {/* {openItem === message?._id ? ( */}
      <tr key={message?._id} className={`hidden-table-data bg-blue-50`}>
        <td colSpan={12} className="px-4 md:px-6">
          <div
            className={`table-hidden-content ${
              openItem === message?._id ? "active" : ""
            }`}
          >
            <div className="content">
              <div className="wish-content border-b-2 py-4">
